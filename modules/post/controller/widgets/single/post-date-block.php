...
class NB_Post_Date{
	...
	public function register_block(){
        ...
        wp_register_script(
            'nextfeed-postdate-block',
            plugin_dir_url(__FILE__) . 'js/post-date.js',
            array('wp-blocks', 'wp-element', 'wp-i18n' )
        );

        register_block_type( 'nextfeed/post-date', [
			'attributes'      => [
                'title' => 'Blog Excerpt'
			],
            'editor_script' => 'nextfeed-postdate-block',
	...
	public function render_block( $attr ){
		...
		if(in_array($post_type, $this->support_posttype() ) ){
			...
			$date =  Render::_instance()->_render_post('blog-date');
		}else{
			$date = esc_html__('Blog Date', 'next-feed' );
        }
        
		ob_start();
        echo sprintf( '<%1$s class="%2$s">%3$s</%1$s>', 'span', 'next-feed-date entry-date', $date );
        wp_reset_postdata();
		return ob_get_clean();
	}
